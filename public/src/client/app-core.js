...
import { fetchLlmConfig, readAppPassword, saveLlmConfig, verifyAppPassword } from "./auth-config.js";
...
(function () {
  ...
  async function init() {
    ...
    els.llmConfigBtn.addEventListener("click", openLlmConfigPanel);
    els.llmConfigForm.addEventListener("submit", handleLlmConfigSubmit);
    els.llmConfigCancelBtn.addEventListener("click", closeLlmConfigPanel);
  ...
  async function checkSavedAuthentication() {
  ...
  function updateAuthOverlay() {
    ...
    const shouldShow = !state.authenticated;
    els.authOverlay.hidden = !shouldShow;
    document.body.classList.toggle("is-auth-locked", shouldShow);
    if (els.authPassword) {
      els.authPassword.disabled = state.authChecking || state.authSubmitting;
    }
    if (els.authSubmitBtn) {
      els.authSubmitBtn.disabled = state.authChecking || state.authSubmitting;
      els.authSubmitBtn.textContent = state.authChecking
        ? "验证中..."
        : state.authSubmitting
          ? "进入中..."
          : "进入";
    }
    if (shouldShow && !state.authChecking && els.authPassword) {
  ...
  async function openLlmConfigPanel() {
    if (state.llmConfigSaving) {
      return;
    }

    state.llmConfigOpen = true;
    updateLlmConfigPanel();
    await loadLlmConfig();
    if (els.llmBaseUrl) {
  ...
  function closeLlmConfigPanel() {
    if (state.llmConfigSaving) {
      return;
    }
    state.llmConfigOpen = false;
    updateLlmConfigPanel();
  }
  ...
  async function loadLlmConfig() {
    ...
    updateLlmConfigPanel();
    try {
      state.llmConfig = await fetchLlmConfig();
      state.authenticated = true;
      updateAuthOverlay();
      fillLlmConfigForm(state.llmConfig);
      setLlmConfigStatus(state.llmConfig.hasApiKey ? "已读取后端大模型配置" : "尚未配置 API Key", state.llmConfig.hasApiKey ? "success" : "warning");
    } catch (error) {
      if (/password|口令|401/i.test(error.message || "")) {
        state.authenticated = false;
        updateAuthOverlay();
      }
      setLlmConfigStatus(error.message || "读取大模型配置失败", "error");
    } finally {
  ...
  function fillLlmConfigForm(config) {
    if (!config) {
      return;
    }
  ...
  async function handleLlmConfigSubmit(event) {
    ...
    const payload = {
      ...
      timeoutMs: els.llmTimeoutMs.value,
    };

    state.llmConfigSaving = true;
    updateLlmConfigPanel();
    setLlmConfigStatus("正在保存大模型配置...", "success");
    try {
      state.llmConfig = await saveLlmConfig(payload);
      fillLlmConfigForm(state.llmConfig);
      setLlmConfigStatus("大模型配置已保存", "success");
      setStatus("大模型配置已保存", "success");
    } catch (error) {
      setLlmConfigStatus(error.message || "保存大模型配置失败", "error");
    } finally {
      state.llmConfigSaving = false;
      updateLlmConfigPanel();
    }
  }

  function updateLlmConfigPanel() {
    if (!els.llmConfigPanel) {
      return;
    }
    els.llmConfigPanel.hidden = !state.llmConfigOpen;
    els.llmConfigBtn.setAttribute("aria-expanded", String(state.llmConfigOpen));
    els.llmConfigBtn.disabled = state.llmConfigLoading || state.llmConfigSaving;
    const disabled = state.llmConfigLoading || state.llmConfigSaving;
    [els.llmBaseUrl, els.llmModel, els.llmApiKey, els.llmTimeoutMs].forEach((input) => {
      if (input) {
        input.disabled = disabled;
      }
    });
    const submit = els.llmConfigForm ? els.llmConfigForm.querySelector('[type="submit"]') : null;
    if (submit) {
      submit.disabled = disabled;
      submit.textContent = state.llmConfigSaving ? "保存中..." : "保存配置";
    }
    if (els.llmConfigCancelBtn) {
      els.llmConfigCancelBtn.disabled = disabled;
    }
  }

  function setLlmConfigStatus(message, type) {
    if (!els.llmConfigStatus) {
      return;
    }
    els.llmConfigStatus.textContent = message || "";
    els.llmConfigStatus.className = `llm-config-status is-${type || "success"}`;
  }
